...
export default function BookCover({ 
  ...
  // Attempt to get cached URL from localStorage immediately
  useEffect(() => {
    // Only run in browser environment
    if (typeof window !== 'undefined' && bookTitle) {
      try {
        const normalizedTitle = bookTitle.trim().toLowerCase();
        const cachedItem = localStorage.getItem(`book_cover_cache_${normalizedTitle}`);
        
        if (cachedItem) {
          const cacheEntry = JSON.parse(cachedItem);
          const now = Date.now();
          const CACHE_DURATION = 7 * 24 * 60 * 60 * 1000; // 7 days (match service)
          
          if (cacheEntry && cacheEntry.url && now - cacheEntry.timestamp < CACHE_DURATION) {
            setCoverUrl(cacheEntry.url);
            setIsLoading(false);
            return; // Skip API call if we have valid cache
          }
        }
      } catch (e) {
        console.warn('Error reading cached cover', e);
  ...
  useEffect(() => {
    // Skip if we already have a cover URL from cache
